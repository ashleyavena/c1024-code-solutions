...
export function Todos() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('/api/todos'); // Use fetch to send a GET request
        if (!response.ok) throw new Error(`response status ${response.status}`);
        const data = (await response.json()) as Todo[]; //  receive & parse the JSON response
        setTodos(data); // update state with the received todos
      } catch (error) {
        setError(error);
      } finally {
        setIsLoading(false);
